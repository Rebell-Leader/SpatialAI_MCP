...
2. **Write `config.vsh.yaml`.** Minimal, validated arguments; clear biological
   descriptions; pin the engine (Docker image + setup packages). Example shape:

   ```yaml
   name: my_method
   namespace: <stage>
   description: "One-line biological purpose."
   arguments:
     - name: "--input"
       type: file
       required: true
       description: "Input SpatialData (zarr)."
     - name: "--output"
       type: file
       direction: output
       required: true
   ...
   (Confirm the exact `__merge__` path and current Viash schema against the live
   repo; Viash config keys evolve between major versions.)
